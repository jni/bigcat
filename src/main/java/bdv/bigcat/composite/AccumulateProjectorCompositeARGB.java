...
{
	public static AccumulateProjectorFactory< ARGBType > factory = ( sourceProjectors, sources, sourceScreenImages, targetScreenImage, numThreads, executorService ) -> new AccumulateProjectorCompositeARGB(
			sourceProjectors,
			sourceScreenImages,
			targetScreenImage,
			numThreads,
	...
	{
		super( sourceProjectors, sources, target, ( t, l ) -> {}, numThreads, executorService );
	}

	@Override
	protected void accumulate( final Cursor< ? extends ARGBType >[] accesses, final ARGBType target )
	{
		// TODO This is bad, we want to be able to paint on top of arbitrary
		// background, e.g.
		// the sky line of NY or some sort of eye pleasing gradient, the outside
		// code should
		// take care for clearing/ preparing the background and not leave it as
		// is.
